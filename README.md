...
- i)The if statement is similar to what we have in python except in Java we use "{ }".
- ii) The curly bracket are different in java, in python we use indentation but in java we use indentation. 
- iii) In python we use the word "and" to show and. While in Java we use "&&".  
- iv) In python we use "print("")" to print out on to the console but on java we do "System.out.println("")"
- V) In java we have to specify the type of variable we are creating and cannot change the type while on python you can change them
